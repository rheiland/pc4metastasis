...
constraints_mets = [ ['K_l = inf'], ['K_l = inf', 'alpha_lg = alpha_lr'], ['K_l = inf', 'phi_r = 3*phi_g'], ['K_l = inf', 'f_Eg = f_Er'], ['K_l = inf', 'alpha_lg = alpha_lr', 'phi_r = 3*phi_g'], ['K_l = inf', 'alpha_lg = alpha_lr', 'f_Eg = f_Er'], ['K_l = inf', 'phi_r = 3*phi_g', 'f_Eg = f_Er'], ['K_l = inf', 'alpha_lg = alpha_lr', 'phi_r = 3*phi_g', 'f_Eg = f_Er'], [], ['alpha_lg = alpha_lr'], ['phi_r = 3*phi_g'], ['f_Eg = f_Er'], ['alpha_lg = alpha_lr', 'phi_r = 3*phi_g'], ['alpha_lg = alpha_lr', 'f_Eg = f_Er'], ['phi_r = 3*phi_g', 'f_Eg = f_Er'], ['alpha_lg = alpha_lr', 'phi_r = 3*phi_g', 'f_Eg = f_Er'] ]
...
def Apply_Constraints(constrain, ParameterDataFrame, ValueParConst = None):
    ...
    for const in constrain:
        ...
        # Checking error
        if (len(RightSide) != 1 and len(RightSide) != 2):
            print(f"Error reading the constraint: {const}! Constraint must be [ParameterName1 = scalar] or [ParameterName1 = ParameterName2] or [ParameterName1 = scalar * ParameterName2]")
            exit(1)
    ...
    if ( ValueParConst is None ):
        ...
        for const in ConstraintSliced:
            if ( len(const) == 3 ):
                ValueParComplete[const[0]] = const[2]*ValueParComplete[const[1]] # [ParameterName1 = ParameterName2] and [ParameterName1 = scalar * ParameterName2]
            else:
...
def MaxLikEst_breast(ID_model, tol_fun, plot=False): #breast tol_fun = 135
    ...
    for it in range(1000):
        ...
        if (result['success'] and result['fun'] < tol_fun): break
    if not plot: return result
    # Plot the approx
    model_mle = forward_model_breast(obs_data.obs_time, result['x'], ID_model)
    ...
    ax.set_title("Breast (RFP and GFP cells)")
    ax.set(xlabel = "Time (days)", ylabel = "Number of cells")
    plt.show()
...
def MaxLikEst_mets(ID_model, theta_breast, modelID_breast, tol_fun, plot=False): #breast tol_fun = 135
    ...
    if not plot: return result
    # Plot the approx
    model_mle = forward_model_mets(obs_data.obs_time, result['x'], ID_model, theta_breast, modelID_breast)
    ...
    ax[0].set_title("Bloodstream (RFP and GFP cells)")
    ax[0].set(xlabel = "Time (days)", ylabel = "Number of cells")
    # Plot Lung
    ...
    ax[1].set_title("Lung (RFP and GFP cells)")
    ax[1].set(xlabel = "Time (days)", ylabel = "Number of cells")
    plt.show()
...
def BayesInference_MCMC_breast(ID_model, tol_fun, fileName, Num_Walkers = 30, Max_Iterations = 1000, mle_range = None):
    ...
    def log_prior(theta): # log of prior distribution [uniform distribution]
        if ( (theta < new_lowerBounds).any() | (theta > new_upperBounds).any() ):
            return -np.inf
        else: return 0.0
    ...
    def log_probability(theta):
        ...
        else: return logPrior + log_likelihood_breast(theta, ID_model)
    ...
    sampler = emcee.EnsembleSampler(Num_Walkers, Num_Dimension, log_probability, threads=10,backend=backend)
...
def BayesInference_MCMC_mets(ID_model, theta_breast, modelID_breast, tol_fun, fileName, Num_Walkers = 30, Max_Iterations = 1000, mle_range = None):
    ...
    def log_probability(theta):
        logPrior = log_prior(theta)
        if not np.isfinite(logPrior): return -np.inf
        else: return logPrior + log_likelihood_mets(theta, ID_model, theta_breast, modelID_breast)
...
def Loading_MCMC_breast(folder, Measure='MLE',burn_in = 100, modelID_specific=None):
    ...
    for modelID in ModelsID:
        ...
        # Randy
        fname = os.path.join('data', 'Model_Candidates','MCMC_Breast%03d.h5'%modelID)
        full_fname = os.path.abspath(fname)
        reader = emcee.backends.HDFBackend(full_fname)
...
def Loading_MCMC_mets(folder,theta_breast, modelID_breast, Measure='MLE',burn_in = 100, modelID_specific=None):
    ...
    for modelID in ModelsID:
        ...
        reader = emcee.backends.HDFBackend(folder+"MCMC_Mets%03d.h5"%modelID)
        thin = int(0.1*burn_in)
        samples = reader.get_chain()
        flat_samples = reader.get_chain(discard=burn_in, flat=True, thin=thin)
        samples_loglik = reader.get_log_prob(discard=burn_in, flat=True, thin=thin)
...
if __name__ == '__main__':
    ...
    # Bloodstream and Lung
    modelID_breast = 14
    theta_breast = [[1.16214193e-01, 2.51329204e+08, 1.16740193e-02, 4.65477289e-02, 4.35470175e-02, 1.29863510e+01]]
    tol_fun_mets = 132
    for modelID in range(int(sys.argv[1]),int(sys.argv[2])+1):
        BayesInference_MCMC_mets(modelID,theta_breast,modelID_breast,tol_fun_mets, fileName=folder+"MCMC_Mets%03d.h5"%modelID, mle_range=0.2) # MCMC - Bayes Inference
